...
class CRN_DSPM(nn.Module):
    def __init__(self):
        super(CRN_DSPM, self).__init__()
        # Main Encoder Part
        self.en = Encoder()
        self.de = Decoder()
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.en1 = nn.Sequential(
            DSPM(1, 16, kernel_size=(1, 3), num_experts=3, stride=(1, 2)),
            nn.BatchNorm2d(16),
            nn.ELU()
        )
        self.en2 = nn.Sequential(
            DSPM(16, 32, kernel_size=(1, 3), num_experts=3, stride=(1, 2)),
            nn.BatchNorm2d(32),
            nn.ELU()
        )
        self.en3 = nn.Sequential(
            DSPM(32, 64, kernel_size=(1, 3), num_experts=3, stride=(1, 2)),
            nn.BatchNorm2d(64),
            nn.ELU()
        )
        self.en4 = nn.Sequential(
            DSPM(64, 128, kernel_size=(1, 3), num_experts=3, stride=(1, 2)),
            nn.BatchNorm2d(128),
            nn.ELU()
        )
        self.en5 = nn.Sequential(
            DSPM(128, 256, kernel_size=(1, 3), num_experts=3, stride=(1, 2)),
            nn.BatchNorm2d(256),
            nn.ELU()
        )
